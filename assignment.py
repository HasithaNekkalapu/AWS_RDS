...
def selectBQuery():
    ...
    cursor.execute('select alias from testdb.tablename where STATE like \'%' + request.form.get('state') + '\' and TOT_ENROLL < '+request.form.get('total')+';')
...
def selectQuery():
    ...
    result = cursor.fetchall()
    cursor.execute('SELECT count(*) FROM testdb.tablename where STATE like \'%' + request.form.get('state') + '\';')
    result2 = cursor.fetchall()

    return render_template('table1.html', tableData=result, rows=result2[0]['count(*)'])

@app.route('/createDB', methods=['GET'])
def createDB():
    file_name = 'C:/RDS_EC2/data.csv'
    ...
    insert_query = """LOAD DATA LOCAL INFILE 'C:/RDS_EC2/data.csv' INTO TABLE testdb.tablename FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"'LINES TERMINATED BY '\n' IGNORE 1 LINES"""
    cursor.execute(insert_query)
    conn.commit()
    cursor.execute('SELECT count(*) FROM testdb.tablename')
    result = cursor.fetchall()
    print(result)
